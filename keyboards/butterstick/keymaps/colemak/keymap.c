...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  [GAME] = LAYOUT_butter(
  	KC_Q,  KC_W,  KC_E,  KC_R,  KC_T, KC_Y, KC_U, KC_UP,  KC_O, KC_ENT,
  	KC_A,  KC_S,  KC_D,  KC_F,  KC_G, KC_H, KC_LEFT, KC_DOWN,  KC_RIGHT, TO(BASE)
  )
};
...
uint32_t processQwerty(bool lookup) {
    ...
    P( LSU | LK | RS | RD,    	SEND_STRING(VERSION); SEND_STRING(__DATE__));
		P( LH | ST1 | RF | RP, 		SEND(KC_DEL));
		P( LR | ST2 | RR | RB, 		SEND(KC_BSPC));
    P( LSD | RZ,   			 	SEND(KC_ENT));
    P( LSU | LFT | RT | RD | LSD | LK | RS | RZ,    SWITCH_LAYER(GAME));
